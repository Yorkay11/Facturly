...
export const ImportProductsModal = ({ open, onClose, onSuccess }: ImportProductsModalProps) => {
  ...
  return (
    <ResponsiveModal
      ...
      contentClassName="rounded-2xl sm:rounded-[20px] border border-border/40 bg-background shadow-2xl shadow-black/5 p-0 overflow-hidden flex flex-col max-h-[97vh]"
                      ...
                      {currentPageData.map((row) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            ...
                            >
                              <SelectTrigger className="h-9 text-[13px] rounded-lg border-0 bg-muted/30">
                                <SelectValue />
                              </SelectTrigger>
