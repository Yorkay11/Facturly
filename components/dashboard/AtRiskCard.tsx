    ...
    <div className={cn("rounded-md border border-amber-200 dark:border-amber-900 bg-card p-3 shadow-sm", className)}>
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <div 
            key={item.id} 
            className="rounded-md border border-amber-200/50 dark:border-amber-900/50 bg-card px-3 py-2 transition-shadow hover:shadow-sm"
          >
            <p className="text-base font-bold text-amber-700 dark:text-amber-400 mb-0.5">{item.value}</p>
            ...
            {item.subItems && item.subItems.length > 0 && (
              <div className="mt-2 grid grid-cols-3 gap-1.5 pt-2 border-t border-amber-100 dark:border-amber-900/30">
                {item.subItems.map((subItem, index) => (
                  <div key={index} className="text-center">
                    <p className="text-xs font-bold text-foreground">{subItem.value}</p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">{subItem.label}</p>
                  </div>
                ))}
              </div>
            )}
