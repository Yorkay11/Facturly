...
const validateFile = (
  ...
  if (accept) {
    ...
    if (!isValidType) {
      return { success: false, message: 'Type de fichier non accepté.' };
    }
  }
  ...
  if (maxSizeInMB) {
    ...
    if (fileSizeInMB > maxSizeInMB) {
      return { success: false, message: `Taille max : ${maxSizeInMB} Mo.` };
    }
  }
...
const Idle = ({ accept, maxSizeInMB, onClick, onDrop, error }: IdleProps) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-zinc-200 font-medium">Déposer un fichier</p>
      <p className="text-xs mt-2 text-zinc-400">Cliquez ou glissez-déposez</p>
      {accept && (
      ...
      {maxSizeInMB && (
        <p className="text-xs text-zinc-400">Max : {maxSizeInMB} Mo</p>
      )}
      {error && (
        <motion.p
          animate={{
...
const Loading = () => {
  ...
  return (
    <div className="h-full w-full bg-black relative border-[3px] border-zinc-900 rounded-xl flex flex-col items-center justify-center gap-4">
      <div
        ref={containerRef}
        className="w-full px-10 flex items-center justify-between relative"
      >
        <motion.span
        ...
        >
          <defs>
            ...
            >
              ...
              <stop offset="15%" stopColor="#00bfff" stopOpacity={1} />
              <stop offset="30%" stopColor="#0080ff" stopOpacity={0.8} />
              <stop offset="50%" stopColor="#0066cc" stopOpacity={0.6} />
              <stop offset="70%" stopColor="#004499" stopOpacity={0.4} />
              <stop offset="85%" stopColor="#002266" stopOpacity={0.2} />
          ...
          <path
            d={path}
            strokeWidth={3}
            stroke="url(#upload-path-gradient)"
            strokeOpacity={1}
            strokeLinecap="round"
          />
        </svg>
      </div>
      <p className="text-xs text-zinc-300 font-light">Envoi en cours...</p>
    </div>
  );
};
              ...
              {Array.from(files)
                ...
                .map((file, index) => (
                  <motion.div
                    ...
                    style={{
                      rotateZ:
                        files.length <= 2 ? index * 15 : -(15 - index * 15),
...
const FileInput = ({
  ...
  const onChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setState('loading');
    try {
      await onFileChange?.(selectedFiles);
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    setState('loading');
    try {
      await onFileChange?.(droppedFiles);
  ...
  return (
    <div
      className={cn('w-[300px] h-[200px] relative overflow-hidden', className)}
    >
      <input
        type="file"
        ref={fileInputRef}
      ...
      <AnimatePresence mode="popLayout" initial={false}>
        ...
        >
          ...
          {state === 'success' && files && (
            <Success files={files} onRemove={handleRemove} />
          )}
        </motion.div>
      </AnimatePresence>
    </div>
