  ...
  const handlePay = async () => {
    ...
    try {
      ...
      toast.success("Facture payée", {
        description: `La facture ${bill.invoice.invoiceNumber} a été marquée comme payée.`,
      });
      router.push("/bills");
    } catch (error) {
      ...
      <div className="space-y-4">
        <Breadcrumb
          items={[
            ...
            { label: "Factures reçues", href: "/bills" },
            { label: "Détails" },
          ]}
          className="text-xs"
        />
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-sm text-foreground/70">
            Facture de {invoice.issuer?.name || "N/A"}. Total dû : {formatCurrency(invoice.totalAmount, invoice.currency)}.
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <Card className="border-primary/20">
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1">
              <p className="text-xs uppercase text-foreground/50">Fournisseur</p>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-foreground/60">
                Payé : {formatCurrency(invoice.amountPaid, invoice.currency)}
              </p>
            </div>
            <div className="space-y-1">
        ...
        <Card className="border-primary/20 self-start">
          <CardHeader>
            ...
            <CardDescription>
              {isPaid ? "Cette facture a été payée." : "Marquer cette facture comme payée."}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {isPaid ? (
              <div className="space-y-2 rounded-lg border border-green-200 bg-green-50 p-4">
                ...
                {bill.payment && (
                  <div className="space-y-1 text-sm text-green-600">
                    <p>Méthode : {bill.payment.method}</p>
                    <p>Date : {formatDate(bill.payment.paidAt)}</p>
                    <p>Montant : {formatCurrency(bill.payment.amount, bill.payment.currency)}</p>
                  </div>
                )}
              </div>
            ) : (
              <div className="space-y-4">
                ...
                >
                  ...
                  {isPaying ? "Traitement..." : "Marquer comme payé"}
                </Button>
              </div>
            )}
          </CardContent>
